...
class RelativeAttentionHead(nn.Module):
    """
    Single head of self-attention with relative positional embedding and causal masking
    """
    ...
    def forward(self, x):
        ...
        rel_indices = torch.arange(-L+1, L, device=x.device) + (self.max_len - 1)
        rel_emb_selected = self.rel_emb(rel_indices)

        M = torch.matmul(Q, rel_emb_selected.transpose(0, 1))

        # apply skewing trick
        S_rel = self.skewing_trick(M, L)
...
class MusicTransformerv2(nn.Module):
    ...
    def __init__(self, vocab_size, n_embd, n_head, n_layer, dropout, max_len):
        ...
        self.pos_emb = nn.Parameter(torch.zeros(1, max_len, n_embd)) #note: might not need this because we already have relative pos emb
        self.dropout = nn.Dropout(dropout)
